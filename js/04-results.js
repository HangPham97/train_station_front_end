...
$(function(){
	...
	$(document).on('click','.next-passenger .right button',function(){
		...
		if(isNaN($('#documentNumber'+x).val())== true || $('#documentNumber'+x).val() == "" ){
		...
		if(firstName==0 && lastName==0 && documentNumber==0 && age==0 && animals==0 && equipment==0 && documentType==0 && addBaggage==0 && citizentship==0){
			$('.next-passenger .left'+x).html("<img src='img/done.png' height='40px' width='40px'><span style='color: #97c16a;font-size:15px;'>Done</span>");
			check[x]="done";
			$('#form'+x).css("background","white");
		}
		else{
			$('.next-passenger .left'+x).html("<img src='img/error.png' height='40px' width='40px'><span style='color: red;font-size:15px;'>Some field are filled in incorrectly</span>");
			check[x]="error";
		}
	})
	var checkStep2=0;
